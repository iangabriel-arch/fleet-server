...
const getAllHirePurchases = asyncHandler(async (req, res) => {
  ...
  const [agreements, total] = await Promise.all([
    HirePurchase.find(filter)
      .populate('vehicle',   'make model year registrationNumber sellingPrice')
      .populate('customer',  'firstName lastName phone nationalId')
      .populate('createdBy', 'name role')
      .sort({ createdAt:-1 }).skip(skip).limit(limitNum).lean(),
    HirePurchase.countDocuments(filter),
  ]);
...
const getHirePurchaseById = asyncHandler(async (req, res) => {
  const agreement = await HirePurchase.findById(req.params.id)
    .populate('vehicle',   'make model year registrationNumber sellingPrice images')
    .populate('customer',  'firstName lastName phone email nationalId drivingLicense address')
    .populate('createdBy', 'name role');
...
const createHirePurchase = asyncHandler(async (req, res) => {
  ...
  if (depositAmount > 0) {
    const receiptNumber = `RCP-${Date.now()}-DEP`;
    await Payment.create({ type:'deposit', customer:customerId, hirePurchase:agreement._id, amount:depositAmount, method:'cash', receiptNumber, status:'completed', notes:'HP deposit payment', recordedBy:req.user._id });
  }
...
const recordInstallmentPayment = asyncHandler(async (req, res) => {
  ...
  agreement.paidMonths      += 1;
  agreement.totalPaid       = parseFloat((agreement.totalPaid + paidAmount).toFixed(2));
  agreement.remainingBalance= parseFloat((agreement.remainingBalance - paidAmount).toFixed(2));

  if (agreement.paidMonths >= agreement.totalMonths || agreement.remainingBalance <= 0) {
    agreement.status         = 'completed';
    agreement.completionDate = new Date();
    await Vehicle.findByIdAndUpdate(agreement.vehicle, { status:'sold', currentHirePurchase:null });
  }

  await agreement.save();
  await Customer.findByIdAndUpdate(agreement.customer, { $inc:{ outstandingBalance:-paidAmount } });

  // Auto-create payment record so it appears in Payments page
  const receiptNumber = `RCP-${Date.now()}-HP${installmentNumber}`;
  const payment = await Payment.create({
    type:         'hire_purchase',
    customer:     agreement.customer,
    hirePurchase: agreement._id,
    amount:       paidAmount,
    method:       paymentMethod,
    receiptNumber,
    status:       'completed',
    notes:        `HP Installment #${installmentNumber}`,
    recordedBy:   req.user._id,
  });

  res.status(200).json({ success:true, message:`Installment #${installmentNumber} payment recorded.`, agreement, payment });
});
...
const repossessVehicle = asyncHandler(async (req, res) => {
  ...
  agreement.repossessionDate   = new Date();
  agreement.repossessionReason = req.body.reason || 'Payment default';
  await agreement.save();
