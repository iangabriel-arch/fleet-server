...
const getAllRentals = asyncHandler(async (req, res) => {
  ...
  const [rentals, total] = await Promise.all([
    Rental.find(filter)
      .populate('vehicle',    'make model year registrationNumber dailyRate')
...
const getRentalById = asyncHandler(async (req, res) => {
  const rental = await Rental.findById(req.params.id)
    .populate('vehicle',   'make model year registrationNumber dailyRate weeklyRate monthlyRate')
    .populate('customer',  'firstName lastName phone email nationalId drivingLicense')
    .populate('createdBy', 'name role');
...
const returnVehicle = asyncHandler(async (req, res) => {
  ...
  if (returnDate > expectedEnd) {
    const lateDays = Math.ceil((returnDate - expectedEnd) / (1000*60*60*24));
    latePenalty = parseFloat((lateDays * rental.dailyRate * 1.5).toFixed(2));
  }
  rental.status        = 'completed';
  rental.actualEndDate = returnDate;
  rental.damageCharge  = damageCharge;
  rental.latePenalty   = latePenalty;
  rental.inspectionNotes = inspectionNotes || null;
  rental.finalCost     = parseFloat((rental.totalCost + damageCharge + latePenalty).toFixed(2));

  // Only set returnMileage if provided
  if (returnMileage) rental.returnMileage = returnMileage;

  // Only set fuelLevel if it's a valid enum value
  const validFuelLevels = ['empty','quarter','half','three_quarter','full'];
  if (fuelLevel && validFuelLevels.includes(fuelLevel)) {
    rental.fuelLevelOnReturn = fuelLevel;
  }

  await rental.save();
  await Vehicle.findByIdAndUpdate(rental.vehicle._id, {
    status:       'available',
    currentRental: null,
    mileage:      returnMileage || rental.vehicle.mileage,
  });
